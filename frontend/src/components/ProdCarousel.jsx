...
const ProdCarousel = () => {
    ...
    return (
        <Box sx={{ display: "flex", alignItems: "center", justifyContent: "center", position: "relative" }}>
            <Button onClick={scrollLeft} sx={{ position: "absolute", top: "25%", left: "5%", height: "50%", zIndex: "1" }}>
                <ChevronLeft fontSize='large' />
            </Button>
            <Box
                width={"90%"}
                sx={{ display: 'flex', overflow: "auto", gap: "1%", position: "relative" }}
                ref={carouselRef}
            >
                {carouselData.map((ele) => (
                    <img key={ele.text} width={"15%"} src={ele.url} alt={ele.text} />
                ))}
            </Box>
            <Button onClick={scrollRight} sx={{ position: "absolute", top: "25%", right: "5%", height: "50%", zIndex: "1" }}>
